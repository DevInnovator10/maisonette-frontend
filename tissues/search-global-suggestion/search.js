...
const SuggestionContainer = styled.section`
  ...
  @media screen and (min-width: ${(props) => props.theme.breakpoint.small}) {
    box-shadow: ${(props) => (props.isActive && '0px 30px 30px 0px #00000029')};
    height: auto;
    position: absolute;
  }
`;
...
export class SearchSuggestion extends Component {
    ...
    if (this.state.active) {
      try {
        await getRacProducts({
          ...
          signal,
          params: { cnt: this.state.isMobile ? 2 : 4 }
        })
          .then(async (d) => {
  ...
  handleOnOutsideClick(e) {
    ...
    if (
      ...
      trackEvent({
        eventCategory: 'Search',
        eventAction: 'Exit Search',
        eventLabel: this.props.globalSearchTerm
      });
                        ...
                        <>
                          <ScopeResults>
                            ...
                            >
                              <SearchTerm
                                ...
                                aria-level="2"
                                onClick={() => {
                                  this.trackFullSearch('Suggestion Link');
                                  this.props.onClose();
                                }}
                              >
                              ...
                              this.state.products.length
                                ? (
                                  this.state.products.map(
                                    (product, i) => (
                                      <ProductCardWrapper onClick={this.props.onClose} key={`global-search-wrapper-${product.maisonette_product_id}`}>
                                        <ProductCard
                          ...
                          >
                            <ViewAllResults
                              ...
                              isLink
                              onClick={() => {
                                this.trackFullSearch('View All Button');
                                this.props.onClose();
                              }}
                            >
