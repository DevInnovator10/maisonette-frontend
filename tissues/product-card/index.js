...
export const ProductCard = (props) => {
  ...
  const [inWishlist, setInWishlist] = useState(() => {
    if (props?.product?.product_id_wishlist) {
      return (wishlistActive?.variant_id ?? 0) === parseInt(props.product.product_id_wishlist, 10);
    }
    return (wishlistActive?.variant_id ?? 0) === parseInt(props.product.objectID, 10);
  });
  ...
  useEffect(() => {
    ...
    setInWishlist(() => {
      if (props?.product?.product_id_wishlist) {
        return (wishlistActive?.variant_id ?? 0)
          === parseInt(props.product.product_id_wishlist, 10);
      }
      return (wishlistActive?.variant_id ?? 0) === parseInt(props.product.objectID, 10);
    });
  ...
  return (
    <Card
      ...
      revamp={props.pdpRecommendations && pdpRevamp}
    >
      <>
