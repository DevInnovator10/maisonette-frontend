...
import { updateCheckout, getCart, nextCheckout } from '../../pages/api';
...
const AddressVerification = (props) => {
  ...
  const updateAddress = (address) => {
    ...
    addressUpdateResponse.then(async (addressUpdateRes) => {
      ...
      if (props.user?.addresses?.length > 0) {
        ...
        if (token) {
          ...
          const addresses = [
            props.cart?.updateShipping, verifiedShipping];
      ...
      if (addressUpdateRes.state === 'delivery' && addressUpdateRes.ship_address && addressUpdateRes.bill_address) {
        // transition order to next state (payment) to use default shipping and calculate tax
        await nextCheckout({ id: addressUpdateRes.number })
          .then((nextRes) => {
            if (!nextRes.errors) {
      ...
      Router.push('/checkout').then(() => props.setCheckoutLoading(false));
    });
  };
        ...
        <VerificationWrapper role="dialog" ref={modalRef}>
          <HeadingWrapper>
            ...
            <Heading element="h2" like="heading-5">
              {
                hasSuggestions ? 'Please review your delivery address' : 'Please select a delivery address'
              }
            </Heading>
